...
from .vaegan import VAEGAN_Gen

class LatentLSTM(nn.Module):
    
    def __init__(self, latent_size, hidden_size, device=None, num_channels=3, cell_type="lstm", num_rnn_layers=1):
        ...
        self.vae = VAEGAN_Gen(latent_size=latent_size,
                              device=device, 
...
#         latent_zs = self.get_latent_zs(input_batch)   # latent_zs: N x T x L
        ...
        zs = self.vae.encode(input_batch)                 # NT x L
        #print(zs.shape)
        zs = zs.view((N, T, self.vae.latent_size))         # N x T x L
        next_zs, next_h = self.rnn(zs, h0)                # N x T x L
        next_zs = next_zs.contiguous().view((N*T, -1))    # NT X L
        ys = self.vae.decoder(next_zs)                     # NT x C x H x W
